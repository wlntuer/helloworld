...
db = pymysql.connect(host='rm-m5ei45v3k72p9ppizvo.mysql.rds.aliyuncs.com',
                     user='wlntuer', password='wlntuer@2016', db='mydata', port=3306, charset='utf8')
...
# compile函数编译正则匹配表达式，re.S代表可以换行匹配，使用findall函数选定数据集，也就是爬取的所有源代码
data = re.compile(pat, re.S).findall(page_text)
